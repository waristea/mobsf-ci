...
begin 
  ...
  request["Authorization"] = mobsf_key
  ...
  response = Net::HTTP.start(uri.hostname, uri.port, :read_timeout => 3600) do |http|
  ...
  response2 = Net::HTTP.start(uri.hostname, uri.port, :read_timeout => 3600) do |http|
    http.request(request)
  end
